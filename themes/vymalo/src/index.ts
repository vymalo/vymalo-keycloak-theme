...
function start() {
	...
	kcHeader.insertAdjacentHTML('afterbegin', vymaloLogo);
...
const newCheckboxHTML = (name: string, input: HTMLInputElement) => `
...
function replaceCheckbox() {
	...
	document
		...
		.forEach((oldCheckboxDiv) => {
			if (oldCheckboxDiv) {
				const innerText = oldCheckboxDiv
					.querySelector('label')
					?.innerText?.trim();
...
function replaceInput() {
	const inputBlocks = document
		.querySelector('.login-page')
		.querySelectorAll('.app-fg');
	inputBlocks.forEach((oldInputDiv, idx) => {
		if (oldInputDiv) {
			const label = oldInputDiv.querySelector('label')?.textContent?.trim();
			const input = oldInputDiv.querySelector(
				'input[type="text"], input[type="password"], input[type="email"]',
			);
			const error = oldInputDiv
				.querySelector('.old-text-message')
				?.textContent?.trim();
			const hiddenInput = oldInputDiv.querySelector(
				'input[type="hidden"]',
			)?.outerHTML;

			if (label && input) {
				oldInputDiv.outerHTML = newInputHtml(
					idx,
					label,
					input,
					error,
...
function replaceOr() {
	...
	kcSocialProvider.insertAdjacentHTML(
		'beforebegin',
		`<div class="divider">${orMsg}</div>`,
	);
